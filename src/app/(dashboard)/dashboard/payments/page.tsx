      ...
      <div className="border border-border rounded-xl bg-card shadow-sm overflow-hidden">
                ...
                payments.map((payment) => (
                  <tr key={payment.paymentId} className="hover:bg-muted/30 transition-colors group">
                    <td className="p-4 font-medium text-foreground">
                      {payment.transactionId || `#S-${payment.paymentId.slice(0, 6)}`}
                    </td>
        ...
        <div className="md:hidden divide-y divide-border/50">
          {isLoading ? (
            ...
            payments.map((payment) => (
              <div key={payment.paymentId} className="p-4 space-y-3">
                <div className="flex justify-between items-start">
                  <div className="flex flex-col">
                    <span className="text-[10px] uppercase text-muted-foreground font-bold flex items-center gap-1">
                      <Hash size={10} /> ID
                    </span>
                    <span className="text-sm font-bold">{payment.transactionId || `#S-${payment.paymentId.slice(0, 6)}`}</span>
                  </div>
